...
def FMNIST_all(c1=3, c2=5, n_epochs=10000, lr=1e-5, b=10, langevin=True, seed=17, act='relu'):
    ...
    net = Net(lr=lr, input_dim=784, n_hid=128, n_hid2=32, output_dim=1, w_dim=1, n_knots=phi.shape[1],
              N_train=2 * n, phi=torch.tensor(phi, dtype=torch.float32), lamb = 10, langevin=langevin,
...
def main():
    ...
    N = 50
    beta = np.zeros([N, 50, 28*28, 128])
    for i in tqdm(range(N)):
        net = FMNIST_all(c1 = 7, c2 = 9, n_epochs = 1000, lr = 1e-6, b = 100, langevin = True, seed = i)
